...
export function buildOrderProcessingText(quote: QuoteRecord): string {
  ...
  const output = [
    ...
    ...equipmentRows.map((row) =>
      `- [${row.id}] ${row.itemName} | Qty: ${row.quantity} | Unit price: ${money(row.unitPrice)} | ${isLease ? "Pricing basis" : "Line total"}: ${money(row.totalPrice)}${text(row.description) ? ` | ${text(row.description)}` : ""} | Assembly: ${quote.metadata.workflowMode !== "major_project" ? "No (standalone)" : requirements.equipment[row.id]?.kind === "assembly" ? "Yes" : requirements.equipment[row.id]?.kind === "standalone" ? "No (standalone)" : "Not confirmed"}${quote.metadata.workflowMode === "major_project" && requirements.equipment[row.id]?.kind === "assembly" && requirements.equipment[row.id].assemblyDetails ? ` | Assembly details: ${requirements.equipment[row.id].assemblyDetails}` : ""}`),
