...
await setField('Proposal #', 'RQ-PDF-PAGEBREAK-HARDFIX');
await setField('Proposal title', 'PDF Page Break Test Proposal Hardfix');
...
await page.screenshot({ path: '_pdf_review/hardfix-verify.png', fullPage: true });
await page.pdf({ path: '_pdf_review/hardfix-verify.pdf', format: 'Letter', printBackground: true, preferCSSPageSize: true });
...
await fs.writeFile('_pdf_review/hardfix-verify-metrics.json', JSON.stringify(metrics, null, 2));
