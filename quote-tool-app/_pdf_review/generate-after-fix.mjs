...
await setField('Proposal #', 'RQ-PDF-PAGEBREAK');
await setField('Proposal title', 'PDF Page Break Test Proposal');
await setField('Proposal date', '2026-04-22');
await setField('Customer name', 'Long Address Customer');
await setField('Contact name', 'Jordan Example');
await setField('Contact phone', '555-1000');
await setField('Contact email', 'jordan@example.com');
await setField('Address line 1', '100 Long Industrial Parkway');
...
// Pad summary/details so address block sits lower on page 2 and would be more likely to break
for (let i = 0; i < 4; i++) await clickByText('button', 'Add customer detail');
await page.evaluate(() => {
  ...
  wrappers.forEach((wrap, idx) => {
    ...
    if (label && value) {
      ...
      set?.call(value, 'This is filler copy to push the proposal information section lower on the page and exercise print pagination rules for the address cards.');
...
const metrics = await page.evaluate(() => {
  ...
  const headings = [...document.querySelectorAll('.proposal-info-page .proposal-address-grid .proposal-mini-heading')].map((el) => (el.textContent || '').trim());
  if (!pageEl || !grid) return { ok: false, body: document.body.innerText.slice(0, 500) };
  ...
  return {
    ...
    headingBreakAfter: document.querySelector('.proposal-info-page .proposal-address-grid .proposal-mini-heading') ? getComputedStyle(document.querySelector('.proposal-info-page .proposal-address-grid .proposal-mini-heading')).breakAfter : null,
  };
});
